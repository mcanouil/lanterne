...
So is `rebuild`'s `keep-labels` argument, along with the two labels suppression refuses rather than drops: one on an image, which cannot be rebuilt, and one on a stepped region itself.
