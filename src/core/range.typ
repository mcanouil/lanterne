...
#let _bad(value, scope) = fail-type(scope, "range", value, _EXPECTED)
...
#let _as-step(text, value, scope) = {
  if text.match(_DIGITS) == none { _bad(value, scope) }
  let step = int(text)
  if step < 1 { _bad(value, scope) }
  step
}

#let _from-string(value, scope) = {
  let parts = value.split("-")
  if parts.len() == 1 {
    let step = _as-step(parts.first(), value, scope)
    return ((from: step, to: step),)
  }
  if parts.len() != 2 { _bad(value, scope) }
  let (low, high) = parts
  ...
  if low == "" and high == "" { _bad(value, scope) }
  if low == "" { return ((from: 1, to: _as-step(high, value, scope)),) }
  if high == "" { return ((from: _as-step(low, value, scope), to: none),) }
  let (start, end) = (_as-step(low, value, scope), _as-step(high, value, scope))
  if end < start {
    fail(
      scope,
      "range ends before it starts; got " + repr(value),
...
/// @category core
/// @returns array
#let parse-range(value, scope) = {
  if type(value) == int {
    if value < 1 { _bad(value, scope) }
    return ((from: value, to: value),)
  }
  if type(value) == array {
    if value.len() == 0 { _bad(value, scope) }
    return value.map(entry => {
      if type(entry) != int or entry < 1 { _bad(value, scope) }
      (from: entry, to: entry)
    })
  }
  if type(value) == str { return _from-string(value, scope) }
  _bad(value, scope)
}
...
/// @category core
/// @returns int
#let first-step(spans) = spans.map(span => span.from).fold(spans.first().from, calc.min)
